...
# (그룹명, 테두리색, [유스케이스…])
GROUPS = [
    ("계정·권한", "#6c8ebf", ["카카오 로그인", "위치 권한 동의", "로그아웃", "계정 삭제", "프로필·색상 변경"]),
    ("5초 촬영·업로드", "#d79b00", ["현재 위치 영상 촬영", "갤러리 영상 선택", "AI 하이라이트 구간 조정", "AI 블러 확인·토글", "업로드 진행 확인", "영상 교체", "영상 삭제"]),
    ("지도 탐색·핫구역", "#82b366", ["내 점령 격자 확인", "격자 영상 목록 열람", "격자 상세 조회", "핫구역 순위 조회", "미방문 격자 추천 받기"]),
    ("미션·이벤트 (신규)", "#b85450", ["주변 미션 칩 탐색", "축제·팝업 상세 확인", "코스 진행도 확인", "미션 격자 촬영·스탬프 획득", "스탬프북 조회", "길찾기로 이동"]),
    ("개인 도감·게임화", "#9673a6", ["수집 요약 조회", "지역별 수집률 조회", "뱃지 목록 확인", "스트릭 확인", "도감 공개범위 변경"]),
    ("안전", "#b85450", ["영상 신고 제출", "신고 처리", "사용자 차단"]),
...
# 액터 → 그룹 연결
ACTORS = [
    ("사용자", 0, ["계정·권한", "5초 촬영·업로드", "지도 탐색·핫구역", "미션·이벤트 (신규)", "개인 도감·게임화", "안전"]),
    ("운영자", 1, ["안전", "운영"]),
    ("스폰서", 2, ["스폰서 활동 (P2)"]),
]
...
GRID = [["계정·권한", "5초 촬영·업로드", "지도 탐색·핫구역"],
