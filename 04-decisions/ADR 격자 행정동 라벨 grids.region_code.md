...
related: ["[[개인 도감 화면 확정 UX·API 설계]]", "[[Region API 예정]]", "[[Collection API 예정]]", "[[FillMap 지도·격자 DB 설계 MVP]]", "[[FillMap API 스펙 통합]]", "[[ADR MSG-167 후속 결정 탐험률 축·격자 표시명·격자 계약]]", "[[지도·공간 데이터 설계 멘토 설명용]]"]
---
