...
class: decision
status: active
source: "raw/confluence/2026-07-24 설계결정 viewport 실시간 전송방식(polling) · 부하 SLO·임계점 (MSG-134) (cf-18219010).md, raw/confluence/2026-07-16 설계결정 viewport 실시간 전송방식(polling) · 부하 SLO·임계점 (MSG-134) (cf-18219010).md, raw/confluence/2026-07-24 설계결정 viewport 실시간 전송방식(polling) · 부하 SLO·임계점 (MSG-134) (cf-18219010) (1).md"
created: 2026-07-16
updated: 2026-08-09
...
> (2026-07-24 §6 추가) MVP 확대로 포함된 핫구역도 polling — Redis ZSET(`ZINCRBY hotzone:{시간버킷}`)이 집계 원본, `ZREVRANGE` 읽기. websocket은 "라이브 UX" 확정 시에만 재검토.
...
- 임계점이 부족할 때 쓸 레버는?
- 핫구역은 전역 데이터인데 왜 push가 아니라 polling인가?
- 핫구역 조회수 집계는 어디에 어떻게 저장되나?
...
- **핫구역도 polling (2026-07-24 추가, §6)**: push 여부의 진짜 기준은 변화 속도 대비 요구 freshness — 핫구역은 분 단위 집계라 30s~1분 stale 무해. 쓰기는 `ZINCRBY hotzone:{시간버킷}`(Redis ZSET이 집계 원본, 캐시 아님, 시간 버킷+TTL), 읽기는 `ZREVRANGE` 상위 N. 전역 데이터라 개인 도감보다 캐싱 유리. websocket 재검토는 "라이브 UX" 기획 확정 시에만, 영구 조회수 통계가 생기면 Redis→DB flush 배치를 별도 티켓으로. 노출 진입은 지도 홈 상단 셀 "핫구역" 칩 — [[기획확정 지도 홈 개편 상단 셀]].
...
## 출처
raw: `raw/confluence/2026-07-24 설계결정 viewport 실시간 전송방식(polling) · 부하 SLO·임계점 (MSG-134) (cf-18219010).md` (이전: `raw/confluence/2026-07-16 설계결정 viewport 실시간 전송방식(polling) · 부하 SLO·임계점 (MSG-134) (cf-18219010).md`)
