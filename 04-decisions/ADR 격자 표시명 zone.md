...
related: ["[[Region API 예정]]", "[[ADR 지도 SDK 네이버 전환]]", "[[갭 분석 디자인 문서 코드 싱크]]", "[[Grid 확장 API 예정]]", "[[ADR MSG-167 후속 결정 탐험률 축·격자 표시명·격자 계약]]", "[[FE 격자 계약 프론트-백 합의]]", "[[MSG-234 상권 작도 결정 공공데이터 검수]]"]
---
...
> **상태 갱신 (2026-07-28)**: 보류가 풀렸다 — 상권 작도를 공공데이터 초안(231건 자동 생성) + 사람 검수로 해결, **MSG-234 WIP 브랜치 재개**(V8__zones.sql). 정본 [[MSG-234 상권 작도 결정 공공데이터 검수]]. FE 행정동 폴백은 zone 시딩 전까지 유지.
