...
related: ["[[Region API 예정]]", "[[ADR 지도 SDK 네이버 전환]]", "[[갭 분석 디자인 문서 코드 싱크]]", "[[Grid 확장 API 예정]]", "[[ADR MSG-167 후속 결정 탐험률 축·격자 표시명·격자 계약]]", "[[FE 격자 계약 프론트-백 합의]]", "[[ADR 장소 검색 카카오 로컬 프록시]]", "[[zone 표시명 데이터 파이프라인 해설]]", "[[MSG-234 상권 작도 결정 공공데이터 검수]]", "[[zone 상권 공공데이터 근거]]", "[[zone 표시명 FE 계약]]"]
---
