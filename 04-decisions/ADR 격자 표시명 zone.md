...
related: ["[[Region API 예정]]", "[[ADR 지도 SDK 네이버 전환]]", "[[갭 분석 디자인 문서 코드 싱크]]", "[[Grid 확장 API 예정]]", "[[ADR MSG-167 후속 결정 탐험률 축·격자 표시명·격자 계약]]", "[[FE 격자 계약 프론트-백 합의]]"]
---
...
> **상태 갱신 (2026-07-23)**: MSG-167 후속에서 재확인·ver 5 디자인 전 화면 채택(표기 예시는 "서면 A-14")됐으나, **MSG-234로 개발 보류**(상권 작도 리소스 미확정) — FE는 행정동 폴백(regionName)만으로 진행. 계약 보존본은 [[FE 격자 계약 프론트-백 합의]] 참조.
