...
- 2026-08-03 — [[ADR 영속 계층 JPA 유지 MyBatis 반려]](04-decisions) 신규: BE 세션 판정(ce-pov Reject) — 네이티브 ~50곳(커스텀 ~70%)은 전부 PostgreSQL 전용 기능 사유, MyBatis 전환은 SQL 위치만 이동·파생 파인더/Hibernate Spatial 상실이라 무득. 번복 트리거는 동적 쿼리 증식(대안은 QueryDSL OpenFeign 포크·JdbcClient 추가). Confluence cf-29917209 발행 + index/hot 갱신.
- 2026-08-03 — 재-sync(08:14Z): cf-29917209(ADR 발행분) 스냅샷 수집·ADR source를 스냅샷 경로로 정합. 멘토링(cf-28999721)·로드맵(cf-29032474) 스냅샷 수집, compiled는 동시 세션(07:29Z) 기수집분 채택 — 02-planning 중복본 미채택, 신기용 멘토링 노트 related에 ADR 역링크 추가. lastSync 08:14Z.
- 2026-08-09 — Confluence sync(lastSync 2026-08-03T08:14Z → 01:43Z, 변경 14건 조회): 데일리 스크럼 6건 제외, JPA 결정(cf-29917209)은 기존 스냅샷과 동일 버전이라 skip, 신규 스냅샷 7건 저장(cf-30179329·30113794·29032474 08-04판·31981569·32636957·33685514·33685556). ingest — 신규 3: [[2026-08-08 김민수 멘토 멘토링]](05-meetings, cf-33685514 + 규호 정리본 cf-33685556) · [[지도·공간 데이터 설계 멘토 설명용]](06-research, cf-31981569) · [[지도 홈 API 연동 가이드 FE]](03-specs, cf-32636957). 08-08 EPSG:5179 권고는 같은 날 [[ADR 격자 계산 EPSG5179 전환]](MSG-347)로 이미 확정·구현됐으므로 회의록은 결정의 입력 기록으로 정리하고, 위경도 규칙을 설명하는 두 신규 노트에는 교체 배너를 달았다. cf-30113794·cf-30179329는 [[Friend API]]·[[Video 공개범위 visibility]]가 이미 BE 정본 기준으로 존재해 신규 노트 없이 예고된 raw 연결만 수행. cf-29032474 08-04판은 [[백엔드 학습 로드맵 Redis HA·알림·관측성]]에 국내 1차 자료 절 추가로 반영. 역링크 9건, Drive 중복 스냅샷 9건 source 병기로 un-ingested 전건 해소. lastSync 01:43Z.
- 2026-08-19 — Confluence 현행화 4건(스펙 통합 전면 재작성 v7 · 명세 v1 배너 · v2 draft 종결 표시 · DB Schema v5 현행화 댓글) 후 sync 4건 ingest. 신규 노트 3(줌아웃 클러스터링 ADR · 행사방 ADR · 0819 박원형 멘토링), 갱신 4(API 스펙 통합·지도 홈 FE 가이드 전면, 명세 v1·v2 draft 배너). Drive raw 1건(박원형 멘토링 AI 요약) 동시 ingest. 8/9자 15건은 본문 무변경으로 재수집 생략. lastSync 2026-08-19T04:35Z.
